...
window._subirPuntosJuego = async function(pts) {
  ...
  try {
    ...
    /* Solo actualiza si el nuevo puntaje supera el récord guardado */
    if (pts > actual) {
...
window.cargarRanking = function() {
  get(ref(db, 'usuarios')).then(snapshot => {
    ...
    if (!snapshot.exists()) { tablaEl.innerHTML = "<p style='text-align:center;color:#aaa'>No hay usuarios aún.</p>"; return; }
    ...
    tablaEl.innerHTML = lista.map((u,i) => {
...
window.cargarRankingJuego = function() {
  get(ref(db, 'usuarios')).then(snapshot => {
    ...
    if (!snapshot.exists()) { tablaEl.innerHTML = "<p style='text-align:center;color:#aaa'>Nadie ha jugado aún.</p>"; return; }
    const uidActual = localStorage.getItem("userUID") || "";
    const medallas  = ["🥇","🥈","🥉"];
    const lista = Object.entries(snapshot.val())
    ...
    if (lista.length === 0) { tablaEl.innerHTML = "<p style='text-align:center;color:#aaa'>Nadie ha jugado aún — ¡sé el primero!</p>"; return; }
    tablaEl.innerHTML = lista.map((u,i) => {
...
/* ===== TICKER — DOS FILAS, una para compras y otra para juego ===== */
